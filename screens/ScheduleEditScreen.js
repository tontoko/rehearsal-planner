...
export default class ScheduleEditScreen extends React.Component {
	constructor (props) {
		...
		if (this.state.location) {
			this.locations = [(<Picker.Item label={this.state.location.name} value={this.state.location} key="0" />), (<Picker.Item label="指定なし" value="" key="1" />)];
		} else {
			this.locations = [(<Picker.Item label="指定なし" value="" key="1" />)];
		}
		// currentUser取得
				...
				snapShot.docs.forEach((e, i) => {
					...
					if (snapShot.docs.length <= i + 1) {
						if (data.id !== this.state.location.id) {
							this.locations = [...this.locations, (<Picker.Item label={data.name} value={data} key={i+2} />)];
						}
